...
# Temperature
T_lcl <- 19 #lower comfort limit (deg C)
T_ucl <- 25 #upper comfort limit (deg C)
T_opt <- 22 #optimal temp (deg C)
...
# Temperature
  #1. If Obs < T_lcl
  mutate(temp = replace (temp, temp < 16, T_lcl)) %>% #note that 16 is not T_lcl
  # 2. If Obs > T_ucl
  mutate(temp = replace(temp, temp > 28, T_ucl)) %>% #note that 28 is not T_ucl
