...
data <- readRDS("~kelseywalker/Desktop/EPIC HOMES/R_thesis/omni_1hour.rds") %>% 
  filter(id_home == '001', location == 'living') %>%
  dplyr::select(location, datetime, date, co2, voc, pm25, temp, humid) %>%
  na.omit(data)
...
# Time Series
```{r}
ggplot(data = results, aes(date, IAPI)) + 
  geom_line() +
  geom_line(aes(date, pm25_percent, color = "pm25")) + 
  geom_line(aes(date, co2_percent, color = "co2")) +
  geom_line(aes(date, voc_percent, color = "voc"))
